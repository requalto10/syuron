...
int main() {
    ...
    output_lines.push_back("Agent Start and Goal Positions:");
    for (int i = 0; i < a; ++i) {
        ...
        output_lines.push_back(agent_info.str());
    }
    ...
    for (int run = 0; run < num_solve; ++run) {
        if (run % 100 == 0) {
            ...
            output_lines.push_back(run_info.str());
        }
    ...
    for (int time_step = 0; time_step <= max_time; ++time_step) {
        ...
        if (time_step % 50 == 0) {
            ...
            output_lines.push_back(progress_msg.str());
        }
    }
