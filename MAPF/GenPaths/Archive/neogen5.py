...
# パラメータの設定
k = 15             # スコアの閾値
time_limit = 200  # 0 ~ time_limitまでのTimeを描画
...
for seg in all_segments:
    ...
    for neighbor_key in neighbor_keys:
        for seg_b in segment_buckets.get(neighbor_key, []):
            if seg_b in processed_segments:
                continue
...
result_msg = f"\nTotal number of segments (num_all_seg): {num_all_seg}\n" \
             f"Number of anonymous segments (num_anony_seg): {num_anony_seg}\n" \
             f"Data residual rate: {data_residual_rate:.4f}%"
print(result_msg)
...
ax.set_ylabel('Y')
ax.set_zlabel('Time')
ax.set_title(f'Agent Paths with High Traffic Segments (Time 0-{time_limit})')
plt.show()
